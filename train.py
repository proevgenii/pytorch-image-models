...
def main():
    ...
    _logger.info(f'{train_loss_fn}=')
    _logger.info(f'{validate_loss_fn}=')
    ...
    if utils.is_primary(args) and args.log_wandb:
        if has_wandb:
            wandb.init(project='ubc_kaggle', entity="proevgenii",config=args)
        else:
            _logger.warning(
    ...
    _logger.info(f'{len(loader_train)=}')
    _logger.info(f'{len(loader_eval)=}')
    start_epoch = 0
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            _logger.info(f'Starting training, epooch:{epoch}')
